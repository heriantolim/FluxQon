function SC_20180525_plot_ion_dynamics_2(varargin)
%% Settings
Groot.usedefault();
...
for i=1:2
	...
	X{i}=S.Time*1e3;
	Y{i,1}=[nan(size(X{i})); S.Number{4}+S.Number{8}; S.Number{9}; S.Number{10}];
	Y{i,2}=S.Fidelity;
	yLim{i,1}=nn(i)*Y_LIM;
	yLim{i,2}=Y_LIM;
end
...
%% Saving
savefigure(fig,'F','ion','dynamics',2,P.CalcMode,...
