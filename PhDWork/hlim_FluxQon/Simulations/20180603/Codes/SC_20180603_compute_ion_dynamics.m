function cID=SC_20180603_compute_ion_dynamics(varargin)

P=inputParser;
P.addRequired('SaveState',@isintegerscalar);
P.addOptional('CalcMode',0,@isintegerscalar);
P.addOptional('NumPhotons',[0,1],...% [mw,op]
	@(x)isintegervector(x) && numel(x)==2);
...
i=2; psi0{2,i}([1,n(1)+1])=psi0{2,i}([n(1)+1,1]);% excite the microwave state
i=3; psi0{2,i}([1,n(2)+1])=psi0{2,i}([n(2)+1,1]);% excite the optical state
...
cID=[{'ion','dynamics',P.CalcMode,n(1),n(2),P.NumIons,P.Site,...
