function SC_20180501_plot_dynamics_1(n,varargin)
%% Settings
Groot.usedefault();
Groot.usedefault('latex',12,.72);

M=2; N=2;
L=Layout.tiledaxes(M,N,[6.4,4],[.1,.1,.1,.1],[.45,.7,.7,.7],[.3,.4,.3,.4]);
...
P.addOptional('LineStrength',.06369,@isrealscalar);% times Grms
...
%% Data Processing
F(1)=FileDir('R','qon','mw','dynamics',n,2,P.Detuning,1,P.Concentration);
F(2)=FileDir('R','ion','mw','dynamics',n,2,P.LineStrength,P.Concentration);
X=cell(M,1);
Y=cell(M,N);
for i=1:M
	S=loaddata(F(i));
