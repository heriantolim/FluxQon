function cID=SC_20180522_compute_ion_dynamics(varargin)
...
ion=runfunction({'S','C',20180521,'default','ion'},1);
P.OpFrequency=diff(ion.Frequency([1,4]));

ion=cell(1,2);
for j=1:2
	ion{j}=runfunction({'S','C',20180521,'default','ion'},P.NumIons,j,...
...
O{2}=Operator.kron(d,1,ion{1}.Number(3));
O{3}=Operator.kron(d,1,ion{1}.Number(4));
O{4}=Operator.kron(d,2,ion{2}.Number(2));
O{5}=Operator.kron(d,2,ion{2}.Number(3));
O{6}=Operator.kron(d,2,ion{2}.Number(4));
O{7}=Operator.kron(d,3,mw.Number);
O{8}=Operator.kron(d,4,op.Number);
O{9}=@(x)fidelity(psi0_op,subdsmat(x,d,3));
...
cID=[{'ion','dynamics',P.CalcMode,n,P.NumIons,...
