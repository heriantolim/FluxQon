...
assert(iscomplexmatrix(H) && diff(size(H))==0,...
	'FluxQon:Operator:round2herm:InvalidInput',...
	'The first argument must be a square complex matrix');
assert(nargin<3,...
	'FluxQon:Operator:round2herm:WrongNargin',...
	'Incorrect number of input arguments.');
if nargin>1
	if isrealscalar(varargin{1}) && varargin{1}>0 && varargin{1}<1
		REL_TOL=varargin{1};
	else
		error('FluxQon:Operator:round2herm:InvalidInput',...
...
if n>=N
	error('FluxQon:Operator:round2herm:ToleranceExceeded',...
