...
if numel(varargin{k})==1
	...
	if D==0
		D=prod(d);
	elseif D~=prod(d)
		error('QuantMech:Operator:Lindblad:InvalidInput',...
	...
	if flag
		try
			...
			if isempty(regexpi(ME1.identifier,':InvalidInput$','once'))
				...
				ME=MException('QuantMech:Operator:Lindblad:InvalidInput',...
...
if k<=K
	warning('QuantMech:Operator:Lindblad:IgnoredInput',...
