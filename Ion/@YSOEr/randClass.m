function x=randClass(varargin)
%% Random Class
%  x=YSOEr.randClass() returns a random value for the class of an YSOEr, which
%    is either 1 or 2. The probability for each value is 0.5.
%
%  x=YSOEr.randClass(N) or x=YSOEr.randClass(N,false) returns N values of random
%    class in a row vector.
%
%  x=YSOEr.randClass(N,true) returns N values of random class where the
...
if nargin==0
	...
	assert(isintegerscalar(N) && N>0,...
		'FluxQon:Ion:YSOEr:randClass:InvalidInput',...
	...
	if nargin>1
		...
		if nargin>2
			error('FluxQon:Ion:YSOEr:randClass:WrongNargin',...
