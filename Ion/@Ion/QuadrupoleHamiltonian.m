function H=QuadrupoleHamiltonian(obj,varargin)
%% Ion Quadrupole Hamiltonian
%  H=obj.QuadrupoleHamiltonian() returns the quadrupole Hamiltonian in the basis
%    of the energy eigenstates, if NumIons=1, or in the basis of the Fock
%    states, otherwise.
%
%  H=obj.ZeemanHamiltonian(basis) returns the quadrupole Hamiltonian in the
...
switch nargin
	...
	case 2
		if ischar(varargin{1}) && isscalar(varargin{1}) ...
			...
			error('FluxQon:Ion:QuadrupoleHamiltonian:InvalidInput',...
				['Input to the basis specifier must be a single character of ',...
					'either ''S'', ''E'', or ''F''']);
		end
	otherwise
		error('FluxQon:Ion:QuadrupoleHamiltonian:WrongNargin',...
...
for k=1:M
	...
	if any(Ti(:)~=0)
		if isscalar(Ti)
			s={'X','Y','Z'};
			for i=1:3
				A=obj.NuclearSpinOperator('S',s{i},MV(k));
				H=H+Ti*A*A;
			end
		else
...
if b>0
	try
		...
		if isempty(regexpi(ME1.identifier,':ToleranceExceeded$','once'))
			...
			warning(...
				'FluxQon:Ion:QuadrupoleHamiltonian:NonHermitian',...
				'The quadrupole Hamiltonian is non Hermitian.');
		end
	end
end
